#!/usr/bin/env python
"""Evaluate NegativeRisk module predictions.

Usage:
    python scripts/evaluate_negative_risk_module.py \
        --predictions artifacts/negative_risk/exp_2026_02/predictions.csv \
        --out-dir reports/local/risk_modules/negative_risk_eval_2026_02
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate NegativeRisk module")
...
def compute_topk_capture(y_true, y_prob, label_name):
    ...
    For negative risk, higher predicted probability means higher risk of
    negative price, so we sort descending by probability and check how many
    actual negatives are captured in the top-k.
    """
    valid = y_true >= 0
    if valid.sum() < 10:
        return []
...
def determine_negative_verdict(topk_rows):
    """Determine NEGATIVE_GO / NEGATIVE_LOW_VALUE / NEGATIVE_NO_GO verdict.

    NEGATIVE_GO: top10% lift >= 2.0 AND recall_at_top20% >= 0.5
    NEGATIVE_LOW_VALUE: any top-k lift >= 1.3
    NEGATIVE_NO_GO: otherwise
    """
    if not topk_rows:
        return "NEGATIVE_NO_GO", "No top-k capture results"
    ...
    # NEGATIVE_LOW_VALUE: any lift >= 1.3
    if max_lift >= 1.3:
        return "NEGATIVE_LOW_VALUE", f"Max lift across top-k = {max_lift:.2f} >= 1.3"

    return "NEGATIVE_NO_GO", f"Max lift across top-k = {max_lift:.2f} < 1.3"
...
def main():
    ...
    label_prob_pairs = [
        ...
        ("deep_negative_label", "deep_negative_prob"),
        ("relative_down_label", "relative_down_prob"),
    ]
    ...
    # ── Verdict ───────────────────────────────────────────────────────
    verdict, reason = determine_negative_verdict(all_topk_rows)
    ...
    # ── GO/NO-GO report ──────────────────────────────────────────────
    go_nogo = f"""# NegativeRisk Evaluation: {verdict}
