"""Tests for spike_risk_features module."""
import numpy as np
import pandas as pd
import pytest
...
class TestFullDayNoLeakage:
    ...
    def test_full_day_lag_features_are_shifted(self):
        ...
        result = build_spike_risk_features(df, mode="FULL_DAY")
        # Lag features should be shifted by 24h, so first 24 rows should have NaN
        lag_cols = ["lagged_price_delta_24h", "lagged_price_delta_48h"]
        for col in lag_cols:
    ...
    def test_previous_day_stats_nan_for_first_day(self):
        df = _make_raw_df(n_days=5)
        result = build_spike_risk_features(df, mode="FULL_DAY")
        if "previous_day_price_delta_mean" in result.df.columns:
            # First 24 rows should be NaN (no previous day data)
            assert result.df["previous_day_price_delta_mean"].iloc[:24].isna().all()
...
class TestFeatureAudit:
    ...
    def test_audit_verdict_with_minimal_data(self):
        ...
        result = build_spike_risk_features(df, mode="FULL_DAY")
        # Should be NOT_READY or PARTIAL_READY (calendar features alone)
...
class TestCalendarFeatures:
    def test_calendar_features_present(self):
        ...
        result = build_spike_risk_features(df, mode="FULL_DAY")
        for feat in SPIKE_CALENDAR_FEATURES:
...
class TestMissingForecastColumns:
    def test_missing_forecast_columns_in_missing_features(self):
        ...
        result = build_spike_risk_features(df, mode="FULL_DAY")
        missing_derived = [f for f in SPIKE_DERIVED_FEATURES if f in result.audit.missing_features]
