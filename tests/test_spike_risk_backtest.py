"""Tests for run_spike_risk_backtest.py.
...
def _make_synthetic_spike_csv(tmp_path, n_days=210, seed=42):
    """Create synthetic hourly CSV with enough spike events.

    Generates data spanning Oct 2025 – Apr 2026.
    Spike events are injected by setting rt_actual >= 500 in some hours.
    """
    ...
    # Inject spike events (rt >= 500) in ~4 % of hours
    n_spike = max(20, int(n_hours * 0.04))
    spike_idx = rng.choice(n_hours, size=n_spike, replace=False)
    rt[spike_idx] = 500 + rng.rand(n_spike) * 400  # rt in [500, 900)

    # Inject extreme spike events (rt >= 800) in ~1.5 % of hours
    n_extreme = max(10, int(n_hours * 0.015))
    extreme_idx = rng.choice(n_hours, size=n_extreme, replace=False)
    rt[extreme_idx] = 800 + rng.rand(n_extreme) * 500  # rt in [800, 1300)

    # Inject relative spike events (rt - da >= 200) in ~3 % of hours
    n_rel = max(15, int(n_hours * 0.03))
    rel_idx = rng.choice(n_hours, size=n_rel, replace=False)
    rt[rel_idx] = da[rel_idx] + 200 + rng.rand(n_rel) * 300
    ...
    csv_path = tmp_path / "synthetic_spike.csv"
...
class TestSpikeRiskBacktest:
    """End-to-end tests for the spike risk backtest script."""

    def test_backtest_runs_end_to_end(self, tmp_path):
        """Backtest runs without error and produces expected output files."""
        csv_path = _make_synthetic_spike_csv(tmp_path)
        out_dir = str(tmp_path / "spike_bt_output")

        from scripts.run_spike_risk_backtest import main

        old_argv = sys.argv
        sys.argv = [
            "run_spike_risk_backtest.py",
            "--data-path", csv_path,
        ...
        assert (out / "spike_risk_backtest_report.md").exists()
    ...
    def test_insufficient_spike_events_handled(self, tmp_path):
        """When spike count < 10, the direction should be marked
        INSUFFICIENT_SPIKE_EVENTS."""
        # Create data with NO spike events
        rng = np.random.RandomState(99)
        n_hours = 24 * 210
        dates = pd.date_range("2025-10-01", periods=n_hours, freq="h")
        da = 300 + rng.randn(n_hours) * 50
        rt = da + rng.randn(n_hours) * 5  # very small noise → no spikes
        ...
        csv_path = str(tmp_path / "no_spikes.csv")
        df.to_csv(csv_path, index=False)

        out_dir = str(tmp_path / "spike_bt_insufficient")
        ...
        if not metrics_df.empty:
            ...
            assert any("INSUFFICIENT" in str(s) for s in statuses), (
                f"Expected INSUFFICIENT_SPIKE_EVENTS status, got: {statuses}"
            )
